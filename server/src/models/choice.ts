...
export interface IChoice extends Document {
  ...
  text: string;
  target_page_id?: mongoose.Types.ObjectId; // Optionnel : peut être défini plus tard lors du développement
  condition?: string;
}
...
const ChoiceSchema: Schema<IChoice> = new Schema<IChoice>({
  ...
  target_page_id: { type: Schema.Types.ObjectId, ref: 'Page', required: false }, // Optionnel
